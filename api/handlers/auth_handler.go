...
func (h *AuthHandler) ResendCode(w http.ResponseWriter, r *http.Request) {
	req, verr := binding.StrictBindJSON[dtos.CodeResendDTO](r)
	...
	data, err := h.authService.ResendCode(ctx, req.Email)
	if err != nil {
		utils.Error(w, http.StatusNotFound, "Code resend failed", err.Error())
		return
	}

	utils.JSON(w, http.StatusOK,"Code resend successfully", data)
}